...
class AppReceitas {
    ...
    private ingredientesAdicionados: string[] = [];
    private favoritosIds: number[];
    private historicoSearches: string[];
    private filtroCategoria: string[] = [];
    private filtroTempoMax = 120;

    private categorias: string[] = [];
    private ingredienteInput!: HTMLInputElement;
    private ingredientesTags!: HTMLElement;
    private recipeGrid!: HTMLElement;
    ...
    private inputSuggestions: HTMLElement | null = null;
    ...
    private setupElementos(): void {
        ...
        this.ingredienteInput = ingredienteInput as HTMLInputElement;
    ...
    private setupEventos(): void {
        this.ingredienteInput.addEventListener('keypress', (e: KeyboardEvent) => {
            if (e.key === 'Enter') {
    ...
    private adicionarIngrediente(): void {
    ...
    private atualizarSugestoes(): void {
        ...
        if (matches.length > 0) {
            this.inputSuggestions.innerHTML = matches
                .map(
                    (ing) => `<div class="suggestion-item" onclick="app.adicionarSugestao('${ing}')">${ing}</div>`
                )
                .join('');
    ...
    public adicionarSugestao(ing: string): void {
        this.ingredienteInput.value = '';
    ...
    private normalizar(ingrediente: string): string {
        return ingrediente.toLowerCase().trim();
    }

    private calcularCompatibilidade(ingredientesRecipe: string[]): CompatResult {
    ...
    private sugerirReceitas(): void {
        ...
        window.setTimeout(() => {
            const receitas = this.recipes
                .map((r) => ({
                    ...r,
                    ...this.calcularCompatibilidade(r.ingredientes),
                }))
    ...
    private exibirResultados(receitas: Recipe[]): void {
    ...
    private criarCartaoReceita(receita: Recipe): HTMLElement {
    ...
    public abrirModal(receitaId: number): void {
        if (!this.modal) {
            return;
        }
    ...
    public toggleFavorito(receitaId: number): void {
    ...
    private carregarFavoritos(): number[] {
    ...
    private salvarHistorico(search: string): void {
